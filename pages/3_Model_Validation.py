...
from services.derived import compute_conditional_breach_probability, compute_transition_matrix, find_worst_basel_window
from theme import apply_theme

st.set_page_config(page_title=f"{APP_TITLE} — Model Validation", layout="wide", initial_sidebar_state="expanded")
apply_theme()
...
# ---------------------------------------------------------------------------
st.markdown('<div class="sentinel-section-title">Statistical Test Results</div>', unsafe_allow_html=True)
        ...
        with col:
            ...
            st.markdown(
                ...
                <div class="sentinel-card">
                    <div class="sentinel-card__label">{test_labels.get(test_key, test_key.title())}</div>
                    <div class="sentinel-card__value">{result['statistic']:.4f}</div>
...
# ---------------------------------------------------------------------------
st.markdown('<div class="sentinel-section-title">Basel Rolling Traffic Light</div>', unsafe_allow_html=True)

basel_df, basel_err = safe_call(
    data_loader.get_basel_traffic_light, selection.asset, method_key, selection.start_date, selection.end_date
)
...
if basel_err:
    ...
    zone_variant = {"GREEN": "green", "YELLOW": "amber", "RED": "red"}.get(latest["zone"], "amber")

    kpi_cols = st.columns(4)
    with kpi_cols[0]:
        st.markdown('<div class="sentinel-card__label">Current Zone</div>', unsafe_allow_html=True)
    ...
    with kpi_cols[1]:
        st.markdown(
            ...
            <div class="sentinel-card">
                ...
                <div class="sentinel-card__value">{latest['risk_multiplier']:.2f}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    with kpi_cols[2]:
        breach_window = int(basel_df["breach_count"].iloc[-1]) if "breach_count" in basel_df else 0
        conditional_prob = compute_conditional_breach_probability(basel_df, window=max(breach_window, 1))
        st.markdown(
            f"""
            <div class="sentinel-card">
                <div class="sentinel-card__label">Conditional Breach Probability</div>
                <div class="sentinel-card__value">{conditional_prob:.2%}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    with kpi_cols[3]:
        ...
        st.markdown(
            f"""
            <div class="sentinel-card">
                <div class="sentinel-card__label">Worst Basel Window</div>
                <div class="sentinel-card__value">{worst.get('zone', '—')}</div>
                <div class="sentinel-card__delta">{worst.get('date', '')}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )

    st.markdown('<div class="sentinel-section-title">Zone Transition Matrix</div>', unsafe_allow_html=True)
    transition_matrix = compute_transition_matrix(basel_df["zone"])
    render_transition_matrix(transition_matrix)

    st.markdown('<div class="sentinel-section-title">Interpretation</div>', unsafe_allow_html=True)
    if latest["zone"] == "RED":
        interpretation = (
        ...
        interpretation = "The model is in the GREEN zone. Breach frequency is within expected statistical bounds."
    st.markdown(f'<div class="sentinel-card">{interpretation}</div>', unsafe_allow_html=True)
